...
class Course(models.Model):
    ...
    summary = models.TextField( blank=True, null=True) #max_length=200 . I delete it to be able to add  more text in admin panel
    ...
    def save(self, *args, **kwargs):
        if not self.slug:  # Generate slug only if it's not set already
            cleaned_title = self.title.strip()
            lowercase_title = cleaned_title.lower()
            normalized_title = unicodedata.normalize('NFKD', lowercase_title)
            slug = re.sub(r'[^\w\s-]', '', normalized_title).strip()
